...
module Babar
  ...
  class Base
    ...
    def query(endpoint, param_map, *args)
    ...
    def query_locations(param_map, *args)
      #FIXME this is actually an array of locations
      query("locations", param_map, *args)
    end
    
    def query_tasks(param_map, *args)
      #Strip off the first element, since it is not a Task
      array_of_json_tasks = query("tasks", param_map, *args)
